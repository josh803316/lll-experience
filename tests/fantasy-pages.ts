...
test('UCSB Legacy app card and GM lab pages', async ({ page }) => {
  ...
  await expect(page.locator('body')).toContainText('All-play ladder')
  await expect(page.locator('body')).toContainText('all-play, no playoffs')
  await expect(page.locator('.fx-brand')).toBeVisible()
  ...
  await page.getByRole('button', { name: 'What Draft grade means' }).first().click()
  await expect(page.getByRole('tooltip')).toContainText('Top fifth A')
  await page.getByRole('button', { name: 'PF/wk' }).click()
  await expect(page.getByRole('button', { name: 'PF/wk' })).toHaveClass(/fx-sort-active/)
  await page.getByRole('button', { name: 'Show all' }).click()

  await page.goto(`${SITE_URL}/fantasy/records`)
  await expect(page.getByRole('heading', { name: 'Trophy Room' })).toBeVisible()
  ...
  await expect(page.locator('nav .fx-tab-active')).toHaveText('GM Lab')
  ...
  await page.locator('.fx-ladder-row').filter({ hasText: 'Jacob' }).first().click()
