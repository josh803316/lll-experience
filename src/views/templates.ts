...
(function() {
  ...
  function buildInfoHtml(chipEl) {
    ...
    if (sc.strengths || sc.weakness || sc.projection || sc.comp) {
      ...
      // Fallback: show per-source rankings when no scouting data is available
      var sr = (PLAYER_DATA[d.name.toLowerCase().trim()] || {}).sourceRanks || {};
      var rows = [['CBS',sr.cbs],['PFF',sr.pff],['ESPN',sr.espn],['NFL',sr.nfl],['Fox',sr.fox]]
        .filter(function(r){ return r[1] && r[1] > 0; })
        .map(function(r){ return '<span class="text-slate-500">' + r[0] + ':</span> <span class="text-slate-300">#' + r[1] + '</span>'; })
        .join('<span class="text-slate-600 mx-1">·</span>');
      if (rows) {
        scoutHtml = '<div class="mt-2 pt-2 border-t border-slate-700 text-xs">'
          + '<div class="text-slate-500 mb-1">Rankings by source</div>'
          + '<div class="flex flex-wrap gap-x-2 gap-y-0.5 leading-snug">' + rows + '</div>'
          + '</div>';
      }
    }
  ...
  function showModal(chipEl) {
    ...
    if (sc.strengths || sc.weakness || sc.projection || sc.comp) {
      ...
      // Fallback: show per-source rankings when no scouting data is available
      var sr2 = (PLAYER_DATA[d.name.toLowerCase().trim()] || {}).sourceRanks || {};
      var rows2 = [['CBS',sr2.cbs],['PFF',sr2.pff],['ESPN',sr2.espn],['NFL',sr2.nfl],['Fox',sr2.fox]]
        .filter(function(r){ return r[1] && r[1] > 0; });
      if (rows2.length) {
        scoutHtml = '<div class="mt-3 rounded-lg bg-gray-50 border border-gray-200 px-3 py-2">'
          + '<div class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Rankings by source</div>'
          + '<div class="grid grid-cols-3 gap-x-4 gap-y-1.5">'
          + rows2.map(function(r){
              return '<div class="flex items-center justify-between">'
                + '<span class="text-xs text-gray-500">' + r[0] + '</span>'
                + '<span class="text-sm font-medium text-gray-900">#' + r[1] + '</span>'
                + '</div>';
            }).join('')
          + '</div>'
          + '</div>';
...
export function leaderboardScoresFragment(
  ...
  const rows = leaderboard
    .map((e, i) => {
      ...
      const onClick = `var self=this;window.__selectedLbUser=${e.user.id};self.closest('tbody').querySelectorAll('tr').forEach(function(r){r.style.background='';r.style.boxShadow='';r.style.outline='';r.querySelectorAll('td').forEach(function(td){td.style.color=''})});self.style.background='#ea580c';self.style.boxShadow='0 0 0 2px #ea580c';self.style.outline='2px solid #ea580c';self.querySelectorAll('td').forEach(function(td){td.style.color='#fff'});setTimeout(function(){var p=document.getElementById('leaderboard-picks-panel');if(p)p.scrollIntoView({behavior:'smooth',block:'start'})},600)`;
      const trAttrs = clickable
        ? ` class="border-b border-gray-200 cursor-pointer ${isPro ? 'bg-blue-50/50 hover:bg-blue-100/50' : 'hover:bg-gray-50'}" data-user-id="${e.user.id}" hx-get="/draft/${year}/leaderboard/picks/${e.user.id}" hx-target="#leaderboard-picks-panel" hx-swap="innerHTML" onclick="${onClick}"`
