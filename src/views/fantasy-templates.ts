...
export function fantasyLiveScoringPanel(data?: FantasyLiveScoringData, error?: string): string {
  ...
  const autoLoad =
    !data && !error
      ? ' hx-post="/fantasy/live-sync" hx-trigger="load" hx-target="this" hx-swap="outerHTML"'
      : ''
...
export function fantasyDashboard(
  ...
  const cohortCards = data.cohorts.map((cohort) => {
    ...
    return `<div style="display:flex;flex-wrap:wrap;gap:6px;${cohort.cohort === 'kid' ? 'justify-content:flex-end' : ''}">${cohort.members.map((member) => `<span class="fx-chip" style="background:${color}14;border:1px solid ${color}33">${escapeHtml(member.displayName)} <b class="fx-number" title="${escapeHtml(`All-time all-play winning percentage for ${member.displayName}`)}">${pct(member.winPct)}</b></span>`).join('')}</div>`
  })
  const dad = data.cohorts.find((cohort) => cohort.cohort === 'dad')
  const kid = data.cohorts.find((cohort) => cohort.cohort === 'kid')
  // Use cross-cohort H2H so the two sides always sum to ~100% (they are inverses).
  const dadPct = dad?.crossWinPct ?? 0
  const kidPct = kid?.crossWinPct ?? 0
  const dadRecord = dad ? record(dad.crossWins, dad.crossLosses, dad.crossTies) : '0–0'
  const kidRecord = kid ? record(kid.crossWins, kid.crossLosses, kid.crossTies) : '0–0'
  ...
  const body = `${nav('all', seasons, latest)}
    <main class="fx-main">
      ...
      <section class="fx-card fx-panel"><div style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap"><div><h2 class="fx-h2" style="font-size:20px">Dads vs. Kids</h2><p class="fx-section-copy">Head-to-head all-play: every Dad vs every Kid each week. Percentages sum to 100%. Member chips show each GM’s own all-time all-play win%.</p></div><span class="fx-number" style="padding:6px 11px;border-radius:999px;color:#f0b429;background:#f0b42922">NEW</span></div><div style="display:flex;justify-content:space-between;margin-top:18px"><div><strong class="fx-hero-value" style="font-size:32px;color:#3fe1a8">${pct(dadPct)} <small class="fx-label" style="font-size:13px">DADS</small></strong><div class="fx-number fx-muted" style="font-size:11px;margin-top:4px">${dadRecord}</div></div><div style="text-align:right"><strong class="fx-hero-value" style="font-size:32px;color:#7c6bff"><small class="fx-label" style="font-size:13px">KIDS</small> ${pct(kidPct)}</strong><div class="fx-number fx-muted" style="font-size:11px;margin-top:4px">${kidRecord}</div></div></div><div class="fx-cohort-bar" style="margin-top:10px"><span style="width:${((dadPct / share) * 100).toFixed(1)}%;background:linear-gradient(90deg,#2bbd8b,#3fe1a8)"></span><span style="width:${((kidPct / share) * 100).toFixed(1)}%;background:linear-gradient(90deg,#7c6bff,#a596ff)"></span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:26px;margin-top:18px">${cohortCards.join('')}</div></section>
