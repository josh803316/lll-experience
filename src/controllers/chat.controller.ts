...
async function getOrCreateDefaultGroup(appId: number, year: number, userId: number) {
  ...
  const existing = await db
    ...
    .from(chatGroups)
    .where(
      and(eq(chatGroups.appId, appId), eq(chatGroups.year, year), eq(chatGroups.isDefault, true))
    )
    .limit(1)
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(chatGroups.appId, appId),
        eq(chatGroups.year, year)
      )
    )
...
async function loadReactionsForMessages(
  ...
  for (const r of rows) {
    ...
    ;(emojiMap.get(r.emoji) as Array<{ userId: number; name: string }>).push({
      userId: r.userId,
      name,
    })
  }
...
function createChatController(config: ChatConfig) {
  return (
    new Elysia({ prefix: config.prefix })
      ...
      .get('/:year/chat', async (ctx: any) => {
        ...
        const app = await getApp(config.appSlug)
        if (!app) {
          ctx.set.status = 404
        ...
        const ticker = config.showTicker
          ? await buildTickerData(app.id, year)
          : { picks: [], draftLive: false, mockActive: false, currentRound: 1 }
        ...
        return chatPage(
          ...
          clerkKey,
          isAdmin,
          { routePrefix: config.prefix, appTitle: config.appTitle, showTicker: config.showTicker }
        )
      })
      ...
      .post(
        ...
        async (ctx: any) => {
          ...
          const [msg] = await db
            .insert(chatMessages)
            .values({ groupId, userId: user.id, content })
            .returning()
      ...
      .post(
        ...
        async (ctx: any) => {
          ...
          ctx.set.headers['HX-Redirect'] = `${config.prefix}/${year}/chat?groupId=${group.id}`
          return ''
        },
        {
...
export const fantasyChatController = createChatController({
  prefix: '/fantasy',
  appSlug: 'ucsb-legacy',
  appTitle: 'UCSB Legacy',
  showTicker: false,
})
