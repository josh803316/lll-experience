...
async function getAvailableYears(appId: number): Promise<number[]> {
  const db = getDB();
  const fromPicks = await db
    .selectDistinct({year: draftPicks.year})
    .from(draftPicks)
    .where(eq(draftPicks.appId, appId));
  const fromSettings = await db
    .selectDistinct({year: draftSettings.year})
    .from(draftSettings)
    .where(eq(draftSettings.appId, appId));
...
function computeScore(
  ...
  officialByPick.forEach((name, pickNum) => {
    if (name) {
      officialByPlayer.set(normalizeName(name), pickNum);
    }
  });
...
function advanceMockIfDue(year: number): void {
  ...
  if (!state?.active || state.revealedCount >= 32) {
    return;
  }
  const now = Date.now();
...
async function buildLeaderboard(appId: number, year: number, officialOverride?: Map<number, string | null>) {
  const db = getDB();
...
export const draftController = new Elysia({prefix: '/draft'})
  .onBeforeHandle((ctx) => {
    ...
    if (path.startsWith('/draft/admin/')) {
      return;
    }
  ...
  .get('/:year/picks', async (ctx: any) => {
    ...
    if (mockState && app) {
      await saveMockStateToDb(mockState, app.id, year);
    }
    ...
    if (app) {
      ...
      if (mockPicks) {
        ...
        officialPicksMap = new Map(
          official.map((r) => [
            r.pickNumber,
            {playerName: r.playerName, position: getPositionForPlayer(r.playerName ?? '', year)},
          ]),
        );
      }
    }
  ...
  .post('/:year/mock/start', async (ctx: any) => {
    ...
    if (state) {
      await saveMockStateToDb(state, app.id, year);
    }
  ...
  .post('/:year/mock/reset', async (ctx: any) => {
    ...
    const app = await getApp('nfl-draft');
    if (app) {
      await deleteMockStateFromDb(app.id, year);
    }
    resetMock(year);
    ctx.set.headers['HX-Redirect'] = `/draft/${year}`;
    return '';
  ...
  .post(
    ...
    async (ctx: any) => {
      ...
      const doubleScorePickNumbers = parsed
        .filter((p) => p.pickNumber > 10 && p.doubleScorePick)
        .map((p) => p.pickNumber);
      const singleDoublePickNum = doubleScorePickNumbers.length > 0 ? Math.min(...doubleScorePickNumbers) : null;
      for (const p of parsed) {
        if (p.pickNumber <= 10) {
          p.doubleScorePick = false;
        } else if (singleDoublePickNum != null && p.pickNumber !== singleDoublePickNum) {
          p.doubleScorePick = false;
        }
      }
        ...
        if (existing) {
          if (playerName) {
            ...
            await db
              ...
              .where(
                and(
                  eq(draftPicks.userId, user.id),
                  eq(draftPicks.appId, app.id),
                  eq(draftPicks.year, year),
                  eq(draftPicks.pickNumber, num),
                ),
              );
          }
        } else if (playerName) {
  ...
  .get('/:year/leaderboard', async (ctx: any) => {
    ...
    const leaderboardYears = [
      CURRENT_DRAFT_YEAR,
      CURRENT_DRAFT_YEAR - 1,
      CURRENT_DRAFT_YEAR - 2,
      CURRENT_DRAFT_YEAR - 3,
    ];
  ...
  .post(
    ...
    async (ctx: any) => {
      ...
      const year = Number(ctx.body?.year) || CURRENT_DRAFT_YEAR;
      const db = getDB();
      const existing = await db
        .select()
        .from(draftSettings)
  ...
  .post(
    ...
    },
    {
      body: t.Object({
        ...
        results: t.Array(
          t.Object({pickNumber: t.Number(), playerName: t.Optional(t.String()), teamName: t.Optional(t.String())}),
        ),
      }),
    },
  )
