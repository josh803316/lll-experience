...
const app = baseApp
  ...
  // sync-draft-picks for scheduled pings (cheaper, guaranteed SELECT).
  .get('/api/cron/keepalive', async ({ request, set }) => {
    ...
    try {
      ...
      return { ok: true, db: true, apps: rows.length, at: new Date().toISOString() }
    } catch (err: unknown) {
      console.error('[CRON] keepalive error:', getErrorMessage(err))
      set.status = 500
  ...
  .get('/api/cron/sync-draft-picks', async ({ request, set }) => {
    ...
    try {
      ...
      try {
        ...
        if (warm.warmed) {
          console.log(`[CRON] analyzer cache warmed → v${warm.version}`)
        }
      } catch (warmErr: any) {
  ...
  .onError(({ error, code, request }) => {
    ...
    if (String(code) === 'NOT_FOUND') {
      ...
      return new Response(
        `<html><body><h1>404 — Not Found</h1><a href="/">Go home</a></body></html>`,
        {
          status: 404,
