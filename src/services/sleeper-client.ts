...
export const sleeperClient = {
  ...
  getNflState: () => sleeperGet<SleeperNflState>('/state/nfl'),
   ...
   * Not under /v1.
   */
  getPlayerWeeklyProjections: async (
    playerId: string,
    season: number
    ...
    const data = await sleeperGetUrl<
      Record<string, SleeperWeeklyProjection> | SleeperWeeklyProjection[] | null
    >(url)
    if (!data) {
      return []
    }
    if (Array.isArray(data)) {
      return data.filter((row) => row?.player_id)
    }
    return Object.values(data).filter((row) => row?.player_id)
  },
}
