...
async function fetchFromESPNSite(year: number): Promise<OfficialPickEntry[]> {
  ...
  const round1 = Array.isArray(rounds) ? rounds.find((r: any) => r?.number === 1 || r?.round === 1) ?? rounds[0] : null;
...
export async function syncOfficialPicksFromMultipleSources(appId: number, year: number): Promise<{ synced: number; source: string }> {
  for (const source of SOURCES) {
    try {
      ...
      for (const p of picks) if (p.pickNumber >= 1 && p.pickNumber <= 32) byNum.set(p.pickNumber, p);
      ...
      for (let num = 1; num <= 32; num++) {
        ...
        const existing = await db
          ...
          .where(and(eq(officialDraftResults.appId, appId), eq(officialDraftResults.year, year), eq(officialDraftResults.pickNumber, num)))
          .limit(1);
...
export async function runDraftAutoTick(year: number): Promise<void> {
  ...
  if (startMs != null && now >= startMs && !draftStarted) {
    ...
    const existing = await db
      ...
      .limit(1);
    if (existing.length > 0) {
      await db.update(draftSettings).set({ draftStartedAt: new Date() }).where(and(eq(draftSettings.appId, appId), eq(draftSettings.year, year)));
    } else {
  ...
  if (draftStarted) {
    ...
    if (synced > 0) console.log("[DRAFT-AUTO] Synced", synced, "official picks from", source, "for year", year);
  }
}
