...
import { isLatePick } from '../config/fantasy-managers.js'
import {
  allPlayFromMatchups,
  applyDraftLetters,
...
describe('UCSB scoring of projected stats', () => {
  ...
  test('DEF uses sack/int buckets, not raw pts_allow', () => {
    ...
    expect(
      scoreStats(def, { sack: 2.54, int: 0.87, pts_allow: 20.5, pts_allow_14_20: 1, def_td: 0.22 })
    ).toBeCloseTo(2.54 + 1.74 + 3 + 1.32)
  })
})
...
describe('positional heat map', () => {
  ...
  test('OVR is the sum of starter columns; a fat bench does not count', () => {
    ...
    const heat = positionalHeatmap(
      [{ rosterId: 1, players: [...starters, { playerId: 'qb-bench', position: 'QB', pts: 3 }] }],
      slots
    )
    const deep = heat[0]
    const starterPts = 20 + 18 + 10 + 22 + 15 + 12 + 9 + 8 + 5
    expect(deep.ovr.pts).toBe(starterPts)
    expect(deep.ovr.pts).toBe(
      deep.qb.pts + deep.rb.pts + deep.wr.pts + deep.te.pts + deep.flex.pts + deep.def.pts
    )
  })
...
describe('player-card counting stats', () => {
  test('averages sources per week then sums the season', () => {
    const totals = blendWeeklyStats(
      [
        { season: 2026, week: 1, playerId: 'a', stats: { rec: 6, rec_yd: 80 } },
        { season: 2026, week: 1, playerId: 'a', stats: { rec: 4, rec_yd: 100 } },
        { season: 2026, week: 2, playerId: 'a', stats: { rec: 5, rec_yd: 50 } },
        { season: 2026, week: 1, playerId: 'b', stats: { rec: 99, rec_yd: 999 } },
        { season: 2025, week: 1, playerId: 'a', stats: { rec: 1, rec_yd: 1 } },
      ],
      2026,
      'a'
    )
    expect(totals.rec).toBeCloseTo(10)
    expect(totals.rec_yd).toBeCloseTo(140)
  })
  test('QB card shows pass line, not rec', () => {
    const stats = countingStatsForPosition('QB', { pass_yd: 4100, pass_td: 32, rec: 1, rec_yd: 12 })
    expect(stats.map((s) => s.label)).toEqual(['Pass Yds', 'Pass TD'])
    expect(stats[0].value).toBe(4100)
  })
})
