...
async function fetchESPNProspects(year: number): Promise<RankedPlayer[]> {
  ...
  for (const item of items) {
    ...
    const name = item?.athlete?.displayName ?? item?.athlete?.shortName ?? item?.displayName ?? null;
    const school = item?.athlete?.college?.displayName ?? item?.college?.displayName ?? item?.school ?? 'Unknown';
    const position =
      item?.athlete?.position?.abbreviation ?? item?.position?.abbreviation ?? item?.positionAbbreviation ?? '?';
    if (!name || !rank) {
      continue;
    }
...
async function fetchESPNSiteProspects(year: number): Promise<RankedPlayer[]> {
  ...
  return prospects
    .map((p: any, i: number) => ({
      ...
      playerName: p?.athlete?.displayName ?? p?.displayName ?? p?.name ?? 'Unknown',
      school: p?.athlete?.college?.displayName ?? p?.college ?? 'Unknown',
 ...
 */
export async function refreshESPNProspects(year: number, force = false): Promise<RefreshResult> {
